...
class UserPop(object):
    ...
    def train_model(self, df, user_col='UserIndex', key_col='keyVector', **unused):

        df_user_pop = df[[user_col, key_col]].groupby(user_col)[key_col].apply(sum).to_frame().reset_index()
        ...
        df_user_pop['Unobserved'] = (df_user_pop[key_col]
                                     .apply(lambda observed_items: np.setdiff1d(np.arange(self.text_dim),
                                                                                observed_items)))
        df_user_pop['predict'] = df_user_pop.apply(lambda x: np.concatenate((x[key_col],x['Unobserved'])), axis=1)
        self.user_pop = df_user_pop.drop(columns=[key_col, 'Unobserved']).values
